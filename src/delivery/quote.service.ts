 ...
 * per-item floor in the cart. Items with a non-positive quantity are skipped
 * entirely, and missing weight/dimensions contribute nothing:
...
export const computeVehicleRequirement = (items: CartItemPhysicals[]): VehicleRequirement => {
  ...
  for (const item of items) {
    if (!(item.quantity > 0)) {
      continue; // OrderItem.quantity has no schema min — never bill a non-positive line
    }
