...
export const inviteVendorUser = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (isDuplicateKeyError(err)) {
      logger.warn(
        `${TAG} duplicate key on invite: ${err instanceof Error ? err.message : String(err)}`,
      );
...
export const removeStaff = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    // The DISABLED write is authoritative (it's what dashboardAuthResolver gates on).
    // The Authentik disable is best-effort (own .catch): failures are logged via
    // logCleanup, but the seat remains disabled in our DB (the user may still be
    // able to authenticate to the IdP while being denied by this API).
